...
public class AirLookupService {
     ...
     */
    public int lookupVenueId(String postcode, String benefitType) {
        AirlookupBenefitToVenue venueName = lookupAirVenueNameByPostCode(postcode);

        String venueByBenefitType = "";

        if (benefitType.equals("pip")) {
            venueByBenefitType = venueName.getPipVenue();
        } else if (benefitType.equals("esa")) {
            venueByBenefitType = venueName.getEsaVenue();
        }
        Integer venueId = getLookupVenueIdByAirVenueName().get(venueByBenefitType);
        return venueId;
    }
}
